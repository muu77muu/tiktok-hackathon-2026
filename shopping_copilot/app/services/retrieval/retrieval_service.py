...
class RetrievalService:
    ...
    async def retrieve(
        ...
        try:
            raw_candidates = await self._run_strategy(query, filters, strategy, top_k)
    ...
    async def _run_strategy(
        self, query: str, filters: dict, strategy: str, top_k: int = DEFAULT_TOP_K
    ) -> list[dict]:
        if strategy == "keyword":
            return await self._keyword(query, filters, top_k)
        if strategy == "vector":
            return await self._vector(query, filters, top_k)
        if strategy == "category":
            return await self._category(filters, top_k)
        if strategy == "hybrid":
            return await self._hybrid(query, filters, top_k)

        logger.warning("unknown retrieval strategy %r, falling back to hybrid", strategy)
        return await self._hybrid(query, filters, top_k)

    async def _hybrid(
        self, query: str, filters: dict, top_k: int = DEFAULT_TOP_K
    ) -> list[dict]:
        tasks = [
            self._keyword(query, filters, top_k),
            self._vector(query, filters, top_k),
        ]

        category = self._extract_category(filters)
        if category:
            tasks.append(self._category(filters, top_k))
    ...
    async def _keyword(
        ...
        if self.keyword_retriever is None:
            return []
        return await self.keyword_retriever.search(
    ...
    async def _category(
        self, filters: dict, top_k: int = DEFAULT_TOP_K
    ) -> list[dict]:
        if self.category_retriever is None:
            return []
        category = self._extract_category(filters)
        return await self.category_retriever.search(
            category, filters=filters, top_k=top_k
        )
